...
In this chapter you'll add global styling and a footer component to the site

== Global CSS

By adding a global CSS file, we avoid repeating common styling for every component.
...
image:common.png[title="Hello page with styling",width=900px]
...
----
+
NOTE: One thing to note here is import of `@enonic/react-components/utils/initPublicPath`.
It sets public path for React4xp to be able to lazy load client components in the environments that don't support automatic public path detection.

We are not entirely done yet, as the Footer component requires some props, in order to work properly (pun intended).

== Static asset

To make things a bit more interesting, the Footer will render the React4XP logo. This means we will need a URL to a static file.
...
image:commonWithFooter.png[title="Hello page with styling",width=900px]
...
You are on a roll, in the next chapter we'll be <<content-type#, rendering content>>.
