...
This describes upgrading to the full feature set of react4xp 7 (for Enonic XP 8), including migrating pages and layouts to react.
...
. On Enonic XP 8, <<api#mount-api, mount the React4XP API>> in each descriptor that renders React4XP (site, admin tool, webapp).
. link:./legacy-upgrade#build-deploy[Deploy^] the application on your existing site.
...
. Make a production build and link:./legacy-upgrade#build-deploy[Deploy^] it.
...
. Make a YAML descriptor file for it so XP knows that it exists, here we can also give it a good description.
+
.src/main/resources/cms/parts/example/example.yaml
[source,YAML]
----
include::{snippet}appendix/upgrade/example.yaml[]
----
...
This is how we migrate it to a react4xp v7 component:
...
For XP to see it we need a descriptor file - in XP 8 this is a YAML file named `Page.yaml`.
This way we can call on it using a descriptor in the component registry and data fetcher.

.src/main/resources/cms/pages/Page/Page.yaml
[source,YAML]
----
include::{snippet}appendix/upgrade/pages/pagev6/Page.yaml[]
----
...
And finally we need to update the descriptor file to match the descriptor name, this is done in the same way as the page.

.src/main/resources/cms/layouts/TwoColumn/TwoColumn.yaml
[source,YAML]
----
include::{snippet}appendix/upgrade/layouts/layoutv6/TwoColumns.yaml[]
----
