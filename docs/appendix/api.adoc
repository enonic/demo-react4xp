...
React4XP serves its compiled assets and SSR endpoints through a dedicated `react4xp` API. From XP 8, that API must be explicitly *mounted* in the descriptor of every context that renders React4XP - otherwise the browser cannot load the assets and rendering fails.

Add the `react4xp` API to the `apis` list of each descriptor where React4XP is used - the `site`, an `admin tool`, and a `webapp`:
