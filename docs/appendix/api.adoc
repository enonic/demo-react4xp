...
React4XP serves its compiled assets and SSR endpoints through a dedicated `react4xp` API, alongside XP's built-in `asset` API. From XP 8, an API must be explicitly *mounted* in the descriptor of every context that renders React4XP - otherwise the browser cannot load the assets and rendering fails.

Add both APIs to the `apis` list of each descriptor where React4XP is used - the `site`, an `admin tool`, and a `webapp`:
...
The `apis` block is identical in all three; only the surrounding descriptor `kind` and fields differ.
