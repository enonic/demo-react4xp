...
NOTE: Required from Enonic XP 8.

React4XP serves its compiled assets and SSR endpoints through a dedicated `react4xp` API.
From XP 8, that API must be explicitly *mounted* in the descriptor of every context that renders React4XP - otherwise the browser cannot load the assets and rendering fails.

Add the `react4xp` API to the `apis` list of the `site` descriptor:

.Site - src/main/resources/cms/site.yaml
[source,yaml]
----
apis:
  - "react4xp"
...
const data = processHtml(params);
----

=== Parameters

The `params` object accepts the same parameters as XP's link:https://developer.enonic.com/docs/xp/stable/api/lib-portal#processhtml[portal.processHtml^], plus `dataFetcher` and `component`:
...
|===
| Name   | Type   | Description
| `value` | string, mandatory | The raw HTML string to process.
| `dataFetcher` | DataFetcher, mandatory | The dataFetcher instance - inside a processor, pass `params.dataFetcher` along.
